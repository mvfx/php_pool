	...
	public function			__construct( array $config )
	{
		if (isset($config['preset']))
		{
				...
				&& is_a($config['vtc'], Vector))
			{
				...
				&& isset($config['far']))
			{
				$r = (float)$config['ratio'];
				$n = (float)$config['near'];
				$f = (float)$config['far'];
				$e = $n / tan(deg2rad((float)$config['fov'] * 0.5));	//Focal Length
		...
		{
			if (isset($config['x']) && is_a($config['x'], Vertex))
				$this->_x = $config['x'];
			else
				$this->_x = new Vertex(['x' => 0, 'y' => 0, 'z' => 0, 'w' => 0]);

			if (isset($config['y']) && is_a($config['y'], Vertex))
				$this->_y = $config['y'];
			else
				$this->_y = new Vertex(['x' => 0, 'y' => 0, 'z' => 0, 'w' => 0]);

			if (isset($config['z']) && is_a($config['z'], Vertex))
				$this->_z = $config['z'];
			else
				$this->_z = new Vertex(['x' => 0, 'y' => 0, 'z' => 0, 'w' => 0]);

			if (isset($config['w']) && is_a($config['w'], Vertex))
				$this->_w = $config['w'];
			else
